...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.menu}><MenuRow icon="book-outline" title="Diario" detail={`${visits} visitas registradas`} onPress={() => router.push('/(tabs)/diary')} /><MenuRow icon="list-outline" title="Listas" detail={`${listCount} listas públicas`} onPress={() => router.push('/lists')} /><MenuRow icon="people-outline" title="Actividad" detail="Sigue a gente con criterio" onPress={() => router.push('/feed')} /><MenuRow icon="map-outline" title="Mapa personal" detail="3 colonias exploradas" onPress={() => router.push('/(tabs)/map')} /><MenuRow icon="shield-checkmark-outline" title="Privacidad" detail="Controla tus datos" last /></View>
    </ScrollView>
  );
}
