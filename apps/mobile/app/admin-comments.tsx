...
export default function AdminCommentsScreen() {
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ['admin-comments', visibility, token], queryFn: () => adminComments(token!, visibility), enabled: Boolean(token && user?.role === 'admin') });
