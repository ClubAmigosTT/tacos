...
export function MapCanvas({ places, active, tacoName, onSelect, userCoordinates, onRegionChangeComplete }: { places: Place[]; active: string; tacoName?: string; onSelect: (id: string) => void; userCoordinates?: { latitude: number; longitude: number }; onRegionChangeComplete?: (coordinates: { latitude: number; longitude: number }) => void }) {
  ...
  return <MapView ref={mapRef} {...(Platform.OS === 'android' ? { provider: PROVIDER_GOOGLE } : {})} style={StyleSheet.absoluteFill} initialRegion={initialRegion} showsUserLocation={Boolean(userCoordinates)} showsMyLocationButton={Platform.OS === 'android'} customMapStyle={mapStyle} onRegionChangeComplete={(region) => onRegionChangeComplete?.({ latitude: region.latitude, longitude: region.longitude })}>{places.map((place) => { const taco = ratingTaco ? place.tacos.find((item) => item.name.toLowerCase() === ratingTaco.toLowerCase()) : undefined; return <Marker key={place.id} coordinate={place.coordinates} onPress={() => onSelect(place.id)}><View style={[styles.pin, active === '92% para mí' && styles.pinAccent]}><Text style={[styles.pinText, active === '92% para mí' && styles.pinTextAccent]}>{taco ? `${taco.name} ${taco.rating.toFixed(1)}` : place.rating.toFixed(1)}</Text></View></Marker>; })}</MapView>;
}
